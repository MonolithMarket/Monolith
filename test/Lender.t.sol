...
contract LenderTest is Test {
    ...
    function test_redeem_reducesTargetBorrowerImmediately() public {
        uint collateralAmount = 5000e18;
        uint borrowAmount = 2000e18;
        uint redeemAmount = 1000e18;
        ...
        uint borrowerCollateralBefore = lender.collateralBalances(borrower);
        uint borrowerDebtBefore = lender.getDebtOf(borrower);
        ...
        assertLt(lender.collateralBalances(borrower), borrowerCollateralBefore, "Borrower collateral should decrease immediately");
        assertLt(lender.getDebtOf(borrower), borrowerDebtBefore, "Borrower debt should decrease immediately");
    }

    function test_redeem_targetsSingleBorrowerOnly() public {
        uint collateralAmount = 5000e18;
        uint borrowAmount = 2000e18;
        uint redeemAmount = 1000e18;
    ...
    function test_repeated_redemptions_bug(int collateralPrice) public {
        ...
        for(uint i; i < 100; i++){
            if (lender.getDebtOf(borrower2) == 0) break;
            vm.startPrank(borrower1);
            lender.adjust(borrower1, int256(collateral.balanceOf(borrower1)), 0);
            (uint price,,) = lender.getCollateralPrice();
            uint borrowingPower = price * lender.collateralBalances(borrower1) * lender.collateralFactor() / 1e18 / 10000 - lender.getDebtOf(borrower1);
            ...
            emit log_uint(lender.collateralBalances(borrower1));
    ...
    function test_repeated_redemptions_fork_fixed() public {
        ...
        uint256 collateralBalance2 = lender.internalToCollateral(lender.collateralBalances(borrower2));
        uint256 debt2 = lender.getDebtOf(borrower2);
        uint256 collateralBalance1 = lender.internalToCollateral(lender.collateralBalances(borrower1));
        ...
        assertEq(lender.internalToCollateral(lender.collateralBalances(borrower2)), 0, "Borrower2's collateral should be zero after update");
        ...
        assertGt(lender.internalToCollateral(lender.collateralBalances(borrower1)), 0, "Borrower1's collateral should be greater than zero after update");
        assertEq(lender.internalToCollateral(lender.collateralBalances(borrower1)), collateralBalance1, "Borrower1's collateral should be unchanged after update");
        ...
        assertEq(lender.internalToCollateral(lender.collateralBalances(borrower1)), collateralBalance1, "Borrower1's collateral should be unchanged before update");
